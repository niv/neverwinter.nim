...
// High level flow:
// 1) Check ABI version.
// 2) Create a compiler instance, give it callbacks into your space. Make sure to also call init.
// 3) Compile a file.
// 4) Service all callbacks - serve files and collect artifacts.
// 5) Receive the compilation artifacts in the write callback.
// 6) Compile more or destroy the compiler instance when done.
//
...
extern "C" CScriptCompiler* scriptCompApiNewCompiler(
    ...
    int bin, // 2010 = ncs
    int dbg, // 2064 = ndb
    ResManWriteToFile resManWriteToFile,
    ResManLoadScriptSourceFile resManLoadScriptSourceFile);
...
extern "C" void scriptCompApiInitCompiler(
    ...
    const char* graphvizOut = nullptr,
    const char* outputAlias = "scriptout");
